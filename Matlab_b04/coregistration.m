...
%  It sets up and runs an SPM batch for aligning a source
%  image to a reference image for the specified subject, with optional
%  images to be coregistered. Reference image is assumed to remain stationary
%  while the source image is moved to match it.
%
% Inputs:
...
%  - other_path: Path to the other source image.
%
% Outputs:
...
function [] = coregistration(ref_path, source_path, other_path)
    ...
    other_img_path = fullfile([other_path,',1']);
    ...
    % Specify any other images to coregister 
     matlabbatch{1}.spm.spatial.coreg.estwrite.other = {other_img_path};
    
    % Coregistration sstimation options
